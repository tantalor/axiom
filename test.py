...
class TestPascal(unittest.TestCase):
  def testZero(self):
    ...
    self.assertTrue(eq(ps.next(), one), "pascal 2,0 = 1")
    self.assertTrue(eq(ps.next(), one), "pascal 3,0 = 1")
  def testOne(self):
    one = next(zero())
    two = next(one)
    three = next(two)
    ...
    self.assertTrue(eq(ps.next(), four),  "pascal 4,1 = 4")
  def testTwo(self):
    one = next(zero())
    three = next(next(one))
    six = add(three, three)
    ...
    self.assertTrue(eq(ps.next(), ten),   "pascal 5,2 = 10")
  def testThree(self):
    one = next(zero())
    ...
    self.assertTrue(eq(ps.next(), twenty), "pascal 6,3 = 20")
  def testThree(self):
    one = next(zero())
